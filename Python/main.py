...
data = pd.concat(objs=[data, elo_rankings], axis=1)
...
beg = datetime(2008, 1, 4)
end = data.Date.iloc[-1]
indices = data[(data.Date > beg) & (data.Date <= end)].index

################### Building of some features based on the past ################

features_player = features_past_generation(
    features_player_creation, 5, "playerft5", data, indices
)
features_duo = features_past_generation(
    features_duo_creation, 150, "duoft", data, indices
)
features_general = features_past_generation(
    features_general_creation, 150, "generalft", data, indices
)
...
features_categorical = data[
    ["Series", "Court", "Surface", "Round", "Best of", "Tournament"]
]
features_categorical_encoded = categorical_features_encoding(
...
features_onehot = pd.concat(
    objs=[features_categorical_encoded, players_encoded, tournaments_encoded],
    axis=1,
)
...
features_elo_ranking = pd.concat(objs=[elo_1, elo_2]).sort_index(kind="merge")

# Categorical features
features_onehot = pd.DataFrame(
    np.repeat(features_onehot.values, 2, axis=0),
    columns=features_onehot.columns,
)
...
start_date = datetime(2015, 1, 5)  # first day of testing set
...
key_matches = np.array(
    [
        test_beginning_match + duration_test_matches * i
        for i in range(int(span_matches / duration_test_matches) + 1)
    ]
)
confs = []
for start in key_matches:
    conf = vibratingAssessStrategyGlobal(
        start,
        10400,
...
conf = pd.concat(objs=confs, axis=0)
...
def profitsAlongTime(conf, matches_delta):
    ...
    milestones = np.array(
        [conf.match.min() + matches_delta * i for i in range(N)]
    )
    profits = []
    lens = []
    for i in range(N - 1):
