...
def features_past_generation(
    features_creation_function, days, feature_names_prefix, data, indices
):
    """
    ...
    for i, match_indice in enumerate(indices):
        ...
        past_matches = data[
            ...
            & (data.Date >= match.Date - datetime.timedelta(days=days))
        ]
        match_features_outcome_1 = features_creation_function(
            1, match, past_matches
        )
        match_features_outcome_2 = features_creation_function(
            2, match, past_matches
        )
    ...
    train.columns = [
        feature_names_prefix + str(i) for i in range(len(train.columns))
    ]
    return train
...
def features_player_creation(outcome, match, past_matches):
    ...
    todo = pd.concat(objs=[wins, losses], axis=0)
    ...
    todo_surface = pd.concat(objs=[wins_surface, losses_surface], axis=0)
    features_player += [
        len(wins_surface),
        len(losses_surface),
        len(todo_surface),
    ...
    per_victory_surface = (
        100 * len(wins_surface) / len(todo_surface)
        if len(todo_surface) > 0
        else np.nan
    )
...
def features_recent_creation(outcome, match, past_matches):
    ...
    nswlmp = (
        todo.iloc[-1, :]["Wsets"] if wlmw == 1 else todo.iloc[-1, :]["Lsets"]
    )
...
def features_duo_creation(outcome, match, past):
    ...
    duo = pd.concat(objs=[duo1, duo2], axis=0)
    features_duo += [len(duo), len(duo1), len(duo2)]
    per_victory_player1 = (
        100 * len(duo1) / len(duo) if len(duo) > 0 else np.nan
    )
...
def features_general_creation(outcome, match, past_matches):
    ...
    best_ranking_as_winner = past_matches[
        (past_matches.Winner == player1)
    ].WRank.min()
    best_ranking_as_loser = past_matches[
        (past_matches.Loser == player1)
    ].LRank.min()
