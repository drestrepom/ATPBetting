...
def xgbModelBinary(xtrain, ytrain, xval, yval, p, sample_weights=None):
    ...
    model = xgb.train(
        ...
        int(p[7]),
        evals=eval_set,
        early_stopping_rounds=int(p[8]),
    )
    return model


def assessStrategyGlobal(
    ...
    # Biggest players :
    biggest_players = data.iloc[range(beg_train, end_train), :][
        ["Winner", "Loser"]
    ]
    biggest_players = pd.concat(
        objs=[biggest_players.Winner, biggest_players.Loser], axis=0
    )
    ...
    to_drop_players = [
        el for el in player_columns if el[7:] not in biggest_players
    ]
    # Biggest Tournaments
    biggest_tournaments = data.iloc[range(beg_train, end_train), :][
        "Tournament"
    ]
    biggest_tournaments = list(
        biggest_tournaments.value_counts().index[:nb_tournaments]
    )
    tournament_columns = [
        el for el in xtrain.columns if el[:10] == "tournament"
    ]
    to_drop_tournaments = [
    ...
    ### ML model training
    model = xgbModelBinary(
        xtrain, ytrain, xval, yval, xgb_params, sample_weights=None
    )
    ...
    p = pd.Series(
        list(
            zip(
                prediction_test_winner,
                prediction_test_loser,
                implied_probabilities.PSW,
                implied_probabilities.PSL,
            )
        )
    )
    ...
    confidenceTest = pd.DataFrame(
        {
            "match": range(beg_test, end_test + 1),
            "win" + model_name: right,
            "confidence" + model_name: confidence,
            "PSW": odds.PSW.values,
        }
    )
    confidenceTest = confidenceTest.sort_values(
        "confidence" + model_name, ascending=False
    ).reset_index(drop=True)

    return confidenceTest


def vibratingAssessStrategyGlobal(
    km,
    dur_train,
    duration_val_matches,
    delta,
    ...
    """
    confTest1 = assessStrategyGlobal(
        km,
        dur_train,
        duration_val_matches,
        delta,
    ...
    confTest2 = assessStrategyGlobal(
        km,
        dur_train - 10,
        duration_val_matches,
        delta,
    ...
    confTest3 = assessStrategyGlobal(
        km,
        dur_train + 10,
        duration_val_matches,
        delta,
    ...
    confTest4 = assessStrategyGlobal(
        km,
        dur_train - 30,
        duration_val_matches,
        delta,
    ...
    confTest5 = assessStrategyGlobal(
        km,
        dur_train + 30,
        duration_val_matches,
        delta,
    ...
    confTest6 = assessStrategyGlobal(
        km,
        dur_train - 45,
        duration_val_matches,
        delta,
    ...
    confTest7 = assessStrategyGlobal(
        km,
        dur_train + 45,
        duration_val_matches,
        delta,
    ...
    if (
        ...
        conf = pd.concat(objs=[confTest1[["match", "PSW"]], c], axis=1)
    ...
    profit = (
        ...
        * (
            matches_selection.PSW[
                matches_selection["win" + model_name] == 1
            ].sum()
